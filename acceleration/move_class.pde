...
// problem going off the boundaries - testing
// working better with boundary cushion
if ((location.x > width-50) || (location.x < 0+50)){
  acceleration.x = acceleration.x * -1;
  velocity.x = velocity.x * -1;}

 //test 2
  //if ((location.x > width-100) || (location.x < 0-100)){
  // acceleration must be reversed at boundary to fix bug with left/right keys
  //acceleration.x = acceleration * -1;
  //velocity.x = velocity.x * -1;}
